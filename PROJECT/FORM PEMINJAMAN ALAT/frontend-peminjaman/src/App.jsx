...
export default function App() {
  ...
  const handlePengembalianSubmit = async (e) => {
    ...
    try {
      const formData = new FormData();
      formData.append("nama", pengembalianForm.nama);
      formData.append("kelas", pengembalianForm.kelas);
      formData.append("nis", pengembalianForm.nis);
      formData.append("noWa", pengembalianForm.noWa);
      formData.append("namaAlat", pengembalianForm.namaAlat);
      formData.append("jumlahAlat", pengembalianForm.jumlahAlat);
      formData.append("tanggalKembali", pengembalianForm.tanggalKembali);
      formData.append("keterangan", pengembalianForm.keterangan);
      if (pengembalianForm.foto) {
        formData.append("foto", pengembalianForm.foto);
      }

      const res = await fetch("http://localhost:8080/kembali", {
        method: "POST",
        body: formData,
      });
      const result = await res.text();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-indigo-100 via-white to-indigo-100 flex">
      <aside className={`w-16 bg-indigo-700 text-white flex flex-col items-center py-6 relative`}>
        <button
            ...
            {activeForm === "peminjaman" && (
              <>
                ...
                <form onSubmit={handlePeminjamanSubmit} className="space-y-6">
                  {[
                    { label: "Nama", name: "nama", description: "Masukkan nama lengkap Anda." },
                    { label: "Kelas", name: "kelas", description: "Masukkan kelas Anda, misalnya: 10A." },
                    { label: "NIS", name: "nis", description: "Nomor Induk Siswa Anda." },
                    { label: "No. WhatsApp", name: "noWa", description: "Masukkan nomor WhatsApp yang aktif." },
                    { label: "Nama Alat", name: "namaAlat", description: "Sebutkan nama alat yang ingin dipinjam." },
                    { label: "Jumlah Alat", name: "jumlahAlat", type: "number", description: "Masukkan jumlah alat yang ingin dipinjam." },
                    { label: "Tanggal Pinjam", name: "tanggalPinjam", type: "date", description: "Pilih tanggal peminjaman." },
                    { label: "Tanggal Kembali", name: "tanggalKembali", type: "date", description: "Pilih tanggal pengembalian." },
                    { label: "Keterangan", name: "keterangan", description: "Berikan keterangan tambahan jika perlu." },
              ...
              <>
                ...
                <form onSubmit={handlePengembalianSubmit} className="space-y-6">
                  {[
                    ...
                    { label: "NIS", name: "nis", description: "Nomor Induk Siswa Anda." },
                    { label: "No. WhatsApp", name: "noWa", description: "Masukkan nomor WhatsApp yang aktif." },
                    { label: "Nama Alat", name: "namaAlat", description: "Sebutkan nama alat yang ingin dikembalikan." },
                    { label: "Jumlah Alat", name: "jumlahAlat", type: "number", description: "Masukkan jumlah alat yang ingin dikembalikan." },
                    { label: "Tanggal Kembali", name: "tanggalKembali", type: "date", description: "Pilih tanggal pengembalian." },
                    { label: "Keterangan", name: "keterangan", description: "Berikan keterangan tambahan jika perlu." },
                  ].map(({ label, name, type = "text", description }) => (
                    <div key={name}>
        ...
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-75 z-20">
            ...
            <span className="ml-4 text-indigo-900 font-semibold">
              Please wait, do not close the application, data is being forwarded to the responsible party.
            </span>
          </div>
        )}
      </main>
